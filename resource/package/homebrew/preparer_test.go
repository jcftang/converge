...
func TestPreparerInterfaces(t *testing.T) {
	t.Parallel()
	assert.Implements(t, (*resource.Resource)(nil), new(homebrew.Preparer))
}
...
func TestPreparerCreatesPackage(t *testing.T) {
	...
	t.Run("when-state-present", func(t *testing.T) {
		p := &homebrew.Preparer{Name: "test1", State: "present"}
		task, err := p.Prepare(context.Background(), fakerenderer.New())
		require.NoError(t, err)
		ashomebrew, ok := task.(*pkg.Package)
		require.True(t, ok)
		assert.Equal(t, "present", string(ashomebrew.State))
	})
	t.Run("when-state-absent", func(t *testing.T) {
		p := &homebrew.Preparer{Name: "test1", State: "absent"}
		task, err := p.Prepare(context.Background(), fakerenderer.New())
		require.NoError(t, err)
		ashomebrew, ok := task.(*pkg.Package)
		require.True(t, ok)
		assert.Equal(t, "absent", string(ashomebrew.State))
	})
	t.Run("when-state-missing", func(t *testing.T) {
		p := &homebrew.Preparer{Name: "test1"}
		task, err := p.Prepare(context.Background(), fakerenderer.New())
		require.NoError(t, err)
		ashomebrew, ok := task.(*pkg.Package)
		require.True(t, ok)
	...
	t.Run("when-name-null", func(t *testing.T) {
		p := &homebrew.Preparer{Name: "", State: "present"}
